...
const updateAnswer = async (req, res) => {
  try {
    ...
    if (!validator.isValidObjectId(answerId)) {
      return res
        .status(400)
        .send({ status: false, message: `${answerId}  is not a valid id` });
    }
    ...
    if (!validator.validString(text)) {
      return res.status(400).send({
        status: false,
        message: " Please provide the text to update.",
      });
    }
  ...
  try {
    ...
    if (!searchQuestion) {
      return res.status(404).send({
        status: false,
        message: `Question doesn't exists or has been deleted.`,
      });
    }
    ...
    if (Array.isArray(fetchAnswers) && fetchAnswers.length === 0) {
      return res.status(404).send({
        status: false,
        message: `No answers found for ${questionId} this question.`,
      });
    }
    return res.status(200).send({
      status: true,
